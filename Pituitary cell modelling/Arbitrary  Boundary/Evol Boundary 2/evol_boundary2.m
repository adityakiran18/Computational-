function [X,Y] =evol_boundary2(x1,y1)
...
eps=2e-6;% Boundary safe distance from each particle
...
X(1,:)=0.87*tempx; %Now scaling them so that they don't coincide with the boundary pts
Y(1,:)=0.87*tempy; %Now Scaling them so that they don't coincide with the boundary pts


% tempx=x1(1:round(length(x1)/N0):end);
% tempy=y1(1:round(length(x1)/N0):end);
% if length(tempx)<N0
%     tempx(end+1) = x1(end);
%     tempy(end+1) = y1(end);
% elseif length(tempx)>N0
%     tempx = tempx(1:N0);
%     tempy = tempy(1:N0);
% end
% X(1,:)=tempx';
...
bdx=repmat(xk,bdsize,1)'-repmat(bxk,N0,1);
bdy=repmat(yk,bdsize,1)'-repmat(byk,N0,1);
rb=sqrt(bdx.^2+bdy.^2);
...
BFy= kboundary*sum( (rb<eps).*bdy ,2);
                ...
                %     X(k,:)=X(k-1,:)+ Fx'*dt/(dragc)  + Boundaryforce(1,:)+   1e-7*sqrt(dt)*randn(1,N0);
                %     Y(k,:)=Y(k-1,:)+ Fy'*dt/(dragc)  + Boundaryforce(2,:)+  1e-7*sqrt(dt)*randn(1,N0);

                
                fprintf('\nBoundary at %gth iter',k);
                display([BFx'; BFy'])
           
                fprintf('\nCell Force at %gth iter',k);
                display([Fx'; Fy']*dt/dragc)    
