...
Fx= sum( (r<4*R0).*dx.*(L0-r)./(r+(r==0)*1),   2);
Fy= sum( (r<4*R0).*dy.*(L0-r)./(r+(r==0)*1),   2);
   ...
   Boundaryforce=zeros(2,N0); %force on each body by the boundary at each iteration              
    ...
    X(k,:)=X(k-1,:)+ mu*Fx'*dt/(dragc)+ Boundaryforce(1,:)+ 1e-7*sqrt(dt)*randn(1,N0);
    Y(k,:)=Y(k-1,:)+ mu*Fy'*dt/(dragc)+ Boundaryforce(2,:)+1e-7*sqrt(dt)*randn(1,N0);
...
% %-----------------------for plotting the balls
        ...
        %magenta, red, green, black, blue, yellow
           ...
           for k=1:Ts
                for i=1:N0
                plot(X(k,i)+R0*cos(t),Y(k,i)+R0*sin(t),'color',randcol(mod(i,6)+1,:),'linewidth',5);
                hold on
                plot(X(k,i),Y(k,i),'+','color','black','linewidth',5); hold on 
